...
export class OverviewComponent implements OnInit, OnDestroy {
  ...
  constructor(private readonly mocksService: MocksService) {
    ...
    this.dataSource.filterPredicate = (data: Mock, text: string) =>
      data.name.indexOf(text) > -1 || data.request.url.indexOf(text) > -1;
    this.subscriptions = [];
  }
  ...
  ngOnInit(): void {
    this.subscriptions.push(
      this.mocksService.getMocks().subscribe((responses) => {
          ...
          switchMap((request: MockRequest) =>
            this.mocksService
              ...
              .pipe(
                tap(() =>
                  this.changed$.next(
                    `Mock '<strong>${request.name}</strong>' has changed the '<strong>delay</strong>' to '<strong>${this.state[request.name].delay}</strong>'`
                  )
                )
              )
          )
          ...
          switchMap((request: MockRequest) =>
            this.mocksService
              ...
              .pipe(
                tap(() =>
                  this.changed$.next(
                    `Mock '<strong>${request.name}</strong>' has changed the '<strong>echo</strong>' to '<strong>${this.state[request.name].echo}</strong>'`
                  )
                )
              )
          )
          ...
          switchMap((request: MockRequest) =>
            this.mocksService
              ...
              .pipe(
                tap(() =>
                  this.changed$.next(
                    `Mock '<strong>${request.name}</strong>' has changed the '<strong>scenario</strong>' to '<strong>${this.state[request.name].scenario}</strong>'`
                  )
                )
              )
          )
